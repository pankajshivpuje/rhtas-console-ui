...
export const ResultsSummary: React.FC<ResultsSummaryProps> = ({ results }) => {
  ...
  return (
    <Flex spaceItems={{ default: "spaceItemsXl" }}>
      {[ConformaResultStatus.Failed, ConformaResultStatus.Warning, ConformaResultStatus.Success].map((status) => (
        <FlexItem key={status}>
          <Content component="p">
            {statusIcon(status)} <strong>{counts[status] ?? 0}</strong> {status}
          </Content>
        </FlexItem>
      ))}
    </Flex>
